...
public class Day02 {
	public static void main(String[] args) {
		Util.log("test checksum: %d", checksum(TEST_CHECKSUM));
		Util.log("prod checksum: %d", checksum(DATA));

		Util.log("test divisors: %d", divisors(TEST_DIVISION));
		Util.log("prod divisors: %d", divisors(DATA));
	}
	...
	private static int divisors(int[][] a) {
		int result = 0;
		for (int j=0; j<a.length; ++j) {
			for (int i0=0; i0<a[j].length-1; ++i0) {
				for (int i1=i0+1; i1<a[j].length; ++i1) {
					int d1 = a[j][i0];
					int d2 = a[j][i1];
					if (d1 < d2) {
						int t = d1;
	...
	private static final int[][] TEST_CHECKSUM = {
			{5, 1, 9, 5},
			{7, 5, 3},
			{2, 4, 6, 8}
	};

	private static final int[][] TEST_DIVISION = {
			{5, 9, 2, 8},
			{9, 4, 7, 3},
			{3, 8, 6, 5}
	};
